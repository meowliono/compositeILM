...
vector<double> diff_log_target(Individual pop[], double a0, double a1, double beta){
    ...
    ay[0] = -log(post_dist_dens(pop, a0, a1, beta));     // record operations that compute ay[0]
...
void leapfrog(Individual pop[], vector<double> z, vector<double> r, double e, int L) {//z-parameters(a0, a1, beta), r-anxillary variable, e-step size, L-number of iterations
    ...
    znxt = z;
    while (znxt[0]<0||znxt[1]<0||znxt[2]<0) {
...
void HMC(Individual pop[], double samples[][3]){
    ...
    */
    ...
    for(size_t i=0; i<50000; i++){
        ...
        r[2] = dis(gen);
        vector<double> z0 = z;
        double H0 = -log(post_dist_dens(pop, z[0], z[1], z[2]))+0.5*(r[0]*r[0]+r[1]*r[1]+r[2]*r[2]);
        leapfrog(pop, z, r, 0.15, 10);
        double Hn = -log(post_dist_dens(pop, z[0], z[1], z[2]))+0.5*(r[0]*r[0]+r[1]*r[1]+r[2]*r[2]);
