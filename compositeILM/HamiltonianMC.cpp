...
vector<double> diff_log_target(Individual pop[], int t_end, double a0, double a1, double beta){
    ...
    ay[0] = -log(post_dist_dens_logprior(pop, t_end, a0, a1, beta));     // record operations that compute ay[0]
...
void leapfrog(Individual pop[], int t_end, vector<double>& z, vector<double>& r, double e, int L) {//z-parameters(a0, a1, beta), r-anxillary variable, e-step size, L-number of iterations
    ...
    vector<double> grad = diff_log_target(pop, t_end,z[0], z[1], z[2]);
    ...
    for (size_t i = 0; i < L; i++) {
        ...
        vector<double> grad = diff_log_target(pop, t_end,znxt[0], znxt[1], znxt[2]);
        //update rnxt
        for (size_t i = 0; i < 3; ++i) {
...
void HMC(Individual pop[], int t_end, double samples[][3]){
    ...
    for(size_t i=0; i<50000; i++){
        ...
        r[2] = dis(gen);
        vector<double> z0 = z;
        double H0 = -log(post_dist_dens_logprior(pop,t_end, z[0], z[1], z[2]))+0.5*(r[0]*r[0]+r[1]*r[1]+r[2]*r[2]);
        leapfrog(pop, t_end,z, r, 0.1, 10);
        //cout <<z[0] <<" "<<z[1]<<" "<<z[2]<<endl;
        double Hn = -log(post_dist_dens_logprior(pop, t_end, z[0], z[1], z[2]))+0.5*(r[0]*r[0]+r[1]*r[1]+r[2]*r[2]);
